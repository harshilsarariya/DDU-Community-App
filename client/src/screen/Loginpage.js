...
const Loginpage = (props) => {
  ...
  var passEye;
    if (pass) {
        passEye = <Entypo name="eye-with-line" style={{position:"absolute", bottom:48, right:16 }} size={20} onPress={() => setPass(false)} />;
    } else {
        passEye = <Entypo name="eye" style={{ position:"absolute", bottom:48, right:16 }} size={20} onPress={() => setPass(true)} />
    }
  return (
    <>
      <View className="bg-white h-full">
        ...
        <View className="py-8 px-8">
          ...
          <View className="relative flex space-y-4 my-8">
            <TextInput placeholder="college id" className="bg-gray-100 rounded-lg px-4 py-2" />
            <TextInput placeholder="password" secureTextEntry={pass} className="bg-gray-100 rounded-lg px-4 py-2" />{passEye}
